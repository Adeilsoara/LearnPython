...
duas funções: uma para fazer a conversão e uma para a saída. Registre a informação A.M./P.M.
...
def converterHora(hora, minuto):
    if hora >=0 and hora <=12:
        print(f'Hora convertida: {hora % 12}:{minuto} AM')
    else:
        print(f'Hora convertida: {hora % 12}:{minuto} PM')
while True:
